...
# Jira Integration.
test-jira:
	@if [ -z "${JIRA_CLOUD_USERNAME}" ]; then echo "Error: JIRA_USERNAME is not defined. Please set JIRA_USERNAME environment variable."; exit 1; fi
	@if [ -z "${JIRA_CLOUD_PASSWORD}" ]; then echo "Error: JIRA_PASSWORD is not defined. Please set JIRA_PASSWORD environment variable."; exit 1; fi
	@if [ -z "${JIRA__CLOUD_URL}" ]; then echo "Error: JIRA_URL is not defined. Please set JIRA_URL environment variable."; exit 1; fi
